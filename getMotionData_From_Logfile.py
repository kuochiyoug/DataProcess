...
if len(argv) != 4:
	print "Usage: getMotionData.py [InputDir] [OutputDir] [DATA_SEQUENCE_YOU_NEED]"
	...
	for f in files:
		...
		for line in InputDatalines:
			if line.startswith("==========Start=========="):
				index_start = InputDatalines.index(line)+1
			elif line.startswith("==========End=========="):
				index_end = InputDatalines.index(line)
			else:
				pass


		InputDatalines_float = map(float,InputDatalines[index_start].split("\t"))
		...
		for i in range(index_end-index_start):
			Data.append(str2flist(InputDatalines[index]))
