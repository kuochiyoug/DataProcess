...
if __name__ == '__main__':
    argv = sys.argv
    if len(argv) != 9:
        print "Usage: Input_image4DNN_connect_Infolder.py [InputFile] [Outputfolder] [ListFile] [ModelFile] [InputLayer] [OutputLayer] [Resized_H] [Resized_W]"
        print len(argv)
        exit()
    ...
    InLayer = int(argv[5])
    OutLayer = int(argv[6])
    Resized_H = int(argv[7])
    Resized_W = int(argv[8])
